...
export type EmanationVariation = {
  ...
  baseR:      number;                           // crystal radius at hull junction
...
export function makeEmanationVariation(seed: number): EmanationVariation {
  const r = prng(seed);
  return {
    scale:      0.90 + r() * 0.55,
    baseR:      0.12 + r() * 0.10,
...
function ring4(c: vec3, fwd: vec3, r: number, tw: number, v: EmanationVariation): vec3[] {
...
function crystalSeg(A:vec3, B:vec3, rA:number, rB:number, uA:number, uB:number, tw:number, v:EmanationVariation, o:number[]): void {
...
export function fillFilamentFaces(t: number, out: number[], v: EmanationVariation): void {
  for (const fil of FILAMENTS) {
    ...
    for (let i = 0; i < L-1; i++) {
      ...
      const rA=v.baseR*(1-t0)+v.tipR*t0, isLast=i===L-2&&!fil.fork;
